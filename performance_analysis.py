# for all the subdirectories of the current directory read the
# density.csv file and calculate the average density
...
# read the density.csv file from 10 subsdirectories of
# agents_performance_analysis and calculate the average density and the
...
def read_density_csv(path, subdirectories, quantities):
    ...
    # create a dictionary with file extensions as keys and the list of outputs
    # as values
...
# df = pd.DataFrame({'Initial density': initial_density, 'Energy cost':
# energy_cost})
...
# create a dataframe with the initial density and the length of the
# density and marginal plot
...
# create a dataframe with the initial density and the final reward and
# marginal plot
...
df = pd.DataFrame(
    {
        'Initial density': initial_density,
        'Reward (only geometry)': reward_final_only_geometry,
    },
)
sns.jointplot(
    x='Initial density',
    y='Reward (only geometry)',
    data=df,
    kind='reg',
)
# plt.show()

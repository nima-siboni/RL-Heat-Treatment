...
class EpsilonGreedyCautious(Exploration):
    ...
    def _get_tf_exploration_action_op(
        ...
        # TODO (Nima): get the hardcoded numbers out :)
        random_valid_action_logits = tf.where(
            ...
            * tf.float32.min,
            tf.repeat([[1.0, 1.0, 1.0, 0.05]], [batch_size], axis=0),
        )

        random_actions = tf.squeeze(
            tf.random.categorical(random_valid_action_logits, 1),
            axis=1,
        )
        ...
        if explore:
            ...
            if isinstance(action_distribution, TorchMultiActionDistribution):
                ...
                random_actions = torch.squeeze(
                    torch.multinomial(random_valid_action_logits, 1),
                    axis=1,
                )
    ...
    @override(Exploration)
    def get_state(self, sess: tf.Session | None = None):
        if sess:

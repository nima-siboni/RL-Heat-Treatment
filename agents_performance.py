...
from utils import create_config, load_training_config, find_the_checkpoint_with_largest_id
# ------------from keras_model import KerasQModel---------------------------------
...
if find_the_latest_agent:
    # TODO: This part should be written better, as currently it finds the directory with the largest checkpoint which
    # is not necessarily the newest one (for example if the check point directories are not cleaned at the beginning
    # of the run
    checkpoint_dir = find_the_checkpoint_with_largest_id(training_config["checkpoint_dir"])
    ...
    checkpoint_dir = 'training_results/checkpoints/checkpoint_003201/checkpoint-3201'

# some good progress before T=1.00005555 and it got ValueError; in that case I didnt stop or anything for the temperature! /home/nima/ray_results/DQN_Furnace_2021-08-17_04-55-17c2rnvcjm/checkpoint_000156/checkpoint-156
assert path.exists(checkpoint_dir), 'the directory for checkpoints does not exist.'
...
for exp_id in (range(nr_repetitions)):
    ...
    T_lst = np.array([obs_lst[i]['temperature'][0] for i in range(len(obs_lst))])
    masses_lst = np.array([np.mean(obs_lst[i]['PF'][:, :, 0] - config['env_config']['shift_PF']) for i in range(len(obs_lst))])
    ...
    os.system("convert -delay 30 -loop 0 " + str(pfs_dir) + "/pf_*.png " + exp_dir + "/pf_evolution.gif")
    plt.plot(T_lst)
    plt.xlabel('steps')
    plt.ylabel('T')
    ...
    plt.close()
    # save the T as csv
    np.savetxt(path.join(exp_dir, 'temperature.csv'), np.array(T_lst), delimiter=',')
    ...
    plt.close()
    np.savetxt(path.join(exp_dir, 'density.csv'), np.array(masses_lst), delimiter=',')
    ...
    plt.close()
    np.savetxt(path.join(exp_dir, 'energy_cost.csv'), np.array(accumulated_energy_cost_lst), delimiter=',')
    ...
    plt.close()
    np.savetxt(path.join(exp_dir, 'accumulated_reward.csv'), np.array(accumulated_reward_lst), delimiter=',')
